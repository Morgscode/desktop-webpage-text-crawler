...
web_scraper.validate_web_url(target_url)
...
for index, link in enumerate(webpage_links):
    ...
    formatted_path = web_scraper.format_path(link)

    new_file_loaction = web_scraper.write_text_to_file(
        page_html_text_content, formatted_path, index, parsed_target_url)

    formatted_text = web_scraper.strip_whitespace_from_file(new_file_loaction)

    web_scraper.write_text_to_file(
